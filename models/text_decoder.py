...
class TextDecoderLayer(nn.Module):
    def __init__(self, 
        ...
        self.MHA_1 = nn.MultiheadAttention(d_model, num_heads, kdim=d_k, vdim=d_v, batch_first =True)
        self.layer_norm1 = nn.LayerNorm(d_model)

        self.k_cross_attn = nn.Linear(vit_dim, d_k)
        self.q_cross_attn = nn.Linear(d_model, d_k)
        self.v_cross_attn = nn.Linear(vit_dim, d_v)

        self.cross_attn = nn.MultiheadAttention(d_model, num_heads_cross_attn, kdim=d_k, vdim=d_v, batch_first =True)
